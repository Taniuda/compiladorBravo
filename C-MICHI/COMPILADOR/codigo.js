...
function ingresarInstruccion() {
    ...
    var instruccion00 = 'si(a >= 5 || a <= 10 && !(chopita == misopa)){// condicion #1} contrario si(variable >= 5){// condicion #2} contrario {// extra}';
    var tipoDato1 = '\npara(entero i = 0; i < 5; i++){ /* bloque de código */ }';
    var instruccion01 = '\ninterruptor(opc){ caso 1 : /* código #1 */ romper; caso 2 : /* código #2 */ romper; caso 3 : /* código #3 */ romper; xdefecto : /* código x */ }';
    var instruccion02 = '\ncomprobado{ /* comentario */ }';
    var instruccion03 = '\nnocomprobado{ /* comentario */ } ';
    var instruccion04 = '\nintenta{ /* comentario */ } atrapar(Excepcion ex){ /* comentario */ }';
    var instruccion05 = '\nx = a + b;';
...
function guardarContenido() {
    ...
    let mensajeErrores = document.getElementById('erroresSintaxis').innerText;

    // Si el mensaje de errores está vacío o no contiene "Sintaxis correcta", analizamos de nuevo
    if (!mensajeErrores.trim() || !mensajeErrores.includes('Sintaxis correcta')) {
        analizar();  // Realiza el análisis
        mensajeErrores = document.getElementById('erroresSintaxis').innerText;  // Actualiza los errores
    }

    // Validamos los errores de sintaxis después de reanalizar
    if (mensajeErrores.includes('Sintaxis correcta')) {
        guardarArchivo(contenido);  // Guarda si la sintaxis es correcta
    } else if (mensajeErrores.includes('Error de Sintaxis en la Línea:')) {
